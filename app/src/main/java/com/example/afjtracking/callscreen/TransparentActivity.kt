    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        when (intent.getStringExtra("type")) {
            "ACCEPT" -> {
                ...
                val acceptIntent = CallIncomingBroadcastReceiver.getIntentAccept(this@TransparentActivity, data)
            ...
            "CALLBACK" -> {
                val data = intent.getBundleExtra("data")
                val acceptIntent = CallIncomingBroadcastReceiver.getIntentCallback(this@TransparentActivity, data)
                sendBroadcast(acceptIntent)
            }
            else -> { // Note the block
                val data = intent.getBundleExtra("data")
