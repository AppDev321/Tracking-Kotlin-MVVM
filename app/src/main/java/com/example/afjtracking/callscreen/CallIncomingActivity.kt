...
import com.example.afjtracking.callscreen.CallIncomingBroadcastReceiver.Companion.ACTION_CALL_INCOMING
import com.example.afjtracking.callscreen.CallIncomingBroadcastReceiver.Companion.EXTRA_CALLKIT_AVATAR
import com.example.afjtracking.callscreen.CallIncomingBroadcastReceiver.Companion.EXTRA_CALLKIT_BACKGROUND_COLOR
import com.example.afjtracking.callscreen.CallIncomingBroadcastReceiver.Companion.EXTRA_CALLKIT_BACKGROUND_URL
import com.example.afjtracking.callscreen.CallIncomingBroadcastReceiver.Companion.EXTRA_CALLKIT_DURATION
import com.example.afjtracking.callscreen.CallIncomingBroadcastReceiver.Companion.EXTRA_CALLKIT_HANDLE
import com.example.afjtracking.callscreen.CallIncomingBroadcastReceiver.Companion.EXTRA_CALLKIT_HEADERS
import com.example.afjtracking.callscreen.CallIncomingBroadcastReceiver.Companion.EXTRA_CALLKIT_INCOMING_DATA
import com.example.afjtracking.callscreen.CallIncomingBroadcastReceiver.Companion.EXTRA_CALLKIT_IS_SHOW_LOGO
import com.example.afjtracking.callscreen.CallIncomingBroadcastReceiver.Companion.EXTRA_CALLKIT_NAME_CALLER
import com.example.afjtracking.callscreen.CallIncomingBroadcastReceiver.Companion.EXTRA_CALLKIT_TEXT_ACCEPT
import com.example.afjtracking.callscreen.CallIncomingBroadcastReceiver.Companion.EXTRA_CALLKIT_TEXT_DECLINE
import com.example.afjtracking.callscreen.CallIncomingBroadcastReceiver.Companion.EXTRA_CALLKIT_TYPE
import com.example.afjtracking.widgets.RippleRelativeLayout
...
class CallIncomingActivity : Activity() {
    ...
    companion object {
        ...
        const val ACTION_ENDED_CALL_INCOMING =
                "com.example.afjtracking.callscreen.ACTION_ENDED_CALL_INCOMING"
    ...
    private fun incomingData(intent: Intent) {
        ...
        if (avatarUrl != null && avatarUrl.isNotEmpty()) {
            ...
            getPicassoInstance(this@CallIncomingActivity, headers)
                    .load(avatarUrl)
    ...
    private fun finishTimeout(data: Bundle?, duration: Long) {
        ...
        val timeStartCall =
                data?.getLong(CallNotificationManager.EXTRA_TIME_START_CALL, currentSystemTime)
                        ?: currentSystemTime
    ...
    private fun initView() {
        ...
        llBackgroundAnimation.layoutParams.height =
                Utils.getScreenWidth() + Utils.getStatusBarHeight(this@CallIncomingActivity)
        ...
        params.setMargins(0, 0, 0, Utils.getNavigationBarHeight(this@CallIncomingActivity))
    ...
    private fun animateAcceptCall() {
        val shakeAnimation =
                AnimationUtils.loadAnimation(this@CallIncomingActivity, R.anim.shake_anim)
    ...
    private fun onAcceptClick() {
        ...
        if (intent != null) {
            val intentTransparent = TransparentActivity.getIntentAccept(this@CallIncomingActivity, data)
            startActivities(arrayOf(intent, intentTransparent))
        } else {
            val acceptIntent = CallIncomingBroadcastReceiver.getIntentAccept(this@CallIncomingActivity, data)
    ...
    private fun onDeclineClick() {
        val data = intent.extras?.getBundle(EXTRA_CALLKIT_INCOMING_DATA)
        val intent =
                CallIncomingBroadcastReceiver.getIntentDecline(this@CallIncomingActivity, data)
