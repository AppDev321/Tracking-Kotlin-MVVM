...
class CallIncomingBroadcastReceiver : BroadcastReceiver() {

    companion object {

        const val ACTION_CALL_INCOMING =
                "ACTION_CALL_INCOMING"
        const val ACTION_CALL_START = "ACTION_CALL_START"
        ...
        fun getIntentIncoming(context: Context, data: Bundle?) =
                Intent(context, CallIncomingBroadcastReceiver::class.java).apply {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (action) {
            ACTION_CALL_INCOMING -> {
                try {
                    callNotificationManager.showIncomingNotification(data)
                    sendEvent(ACTION_CALL_INCOMING, data)
                    addCall(context, Data.fromBundle(data))
                    val soundPlayerServiceIntent = Intent(context, CallSoundPlayerService::class.java)
            ...
            ACTION_CALL_START -> {
                try {
                    sendEvent(ACTION_CALL_START, data)
            ...
            ACTION_CALL_ACCEPT -> {
                try {
                    sendEvent(ACTION_CALL_ACCEPT, data)
                    context.stopService(Intent(context, CallSoundPlayerService::class.java))
                    callNotificationManager.clearIncomingNotification(data)
                    addCall(context, Data.fromBundle(data), true)
                } catch (error: Exception) {
                    error.printStackTrace()
                }
            ...
            ACTION_CALL_DECLINE -> {
                try {
                    sendEvent(ACTION_CALL_DECLINE, data)
                    context.stopService(Intent(context, CallSoundPlayerService::class.java))
                    callNotificationManager.clearIncomingNotification(data)
            ...
            ACTION_CALL_ENDED -> {
                try {
                    sendEvent(ACTION_CALL_ENDED, data)
                    context.stopService(Intent(context, CallSoundPlayerService::class.java))
                    callNotificationManager.clearIncomingNotification(data)
                    removeCall(context, Data.fromBundle(data))
                } catch (error: Exception) {
            ...
            ACTION_CALL_TIMEOUT -> {
                try {
                    sendEvent(ACTION_CALL_TIMEOUT, data)
                    context.stopService(Intent(context, CallSoundPlayerService::class.java))
                    if (data.getBoolean(EXTRA_CALLKIT_IS_SHOW_MISSED_CALL_NOTIFICATION, true)) {
                        callNotificationManager.showMissCallNotification(data)
                    }
                    removeCall(context, Data.fromBundle(data))
                } catch (error: Exception) {
                    error.printStackTrace()
            ...
            ACTION_CALL_CALLBACK -> {
                try {
                    callNotificationManager.clearMissCallNotification(data)
                    sendEvent(ACTION_CALL_CALLBACK, data)
     ...
     fun sendEvent(event: String, data: Bundle) {
        val android = mapOf(
        ...
        val forwardData = mapOf(
         ...
         CallkitIncomingPlugin.sendEvent(event,forwardData)
    }
}